...
```{r mean_total_steps, echo = TRUE, results = "hide", message=FALSE, warning = F}
library(dplyr)
...
The total number of missing numbers in the dataset is **`r sum(bool_nas)`**.
...
```{r factor_variable, results = "hide"}
fac_var <- function(d) {
    tmp <- "weekend"
...
```{r make_panel, results = "hide", warning = F, message = F}
library(ggplot2)
